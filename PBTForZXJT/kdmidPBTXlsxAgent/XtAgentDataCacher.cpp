...
import datetime\n\
\n\
DataList = ['业务日期', '委托日期', '交易日期']\n\
TimeList = ['成交时间']\n\
\n\
files = os.listdir('"+ queryPath +"')\n\
for tmpFile in files:\n\
    if tmpFile.endswith('.csv') or tmpFile.endswith('.CSV'):\n\
        os.remove('"+ queryPath +"' + tmpFile)\n\
for tmpFile in files:\n\
    if tmpFile.endswith('.xlsx') or tmpFile.endswith('.XLSX') or tmpFile.endswith('.xls') or tmpFile.endswith('.XLS'):\n\
