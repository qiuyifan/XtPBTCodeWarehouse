...
namespace agent
{
    ...
    bool XtAgentCSVApi::loadNamePos(string line , vector<string>& headName, const string& findKey)
    {
        bool isFind = false;
        ...
        for (int i = 0; i < lineParm.size(); ++i)
        {
            ...
            if (!findKey.empty() && posName.find(findKey) != string::npos)
            {
                isFind = true;
            }
            if ( i >= headName.size())
    ...
    void XtAgentCSVApi::getDataFromFile(const string& file, const int& funcNo, map < string, vector< map< string, string > > >& retData)
    {
        ...
        if (!dataTemp.empty())
        {
            ...
            {
                bool result = loadNamePos(dataTemp[pos] ,namepos, findKey);
                // 文件校验，如果没找到制定的头，就跳过这个文件,只对第一行进行校验
                if (!result && 0 == pos)
                {
                    return;
                }
            ...
            for (int pos = headLenth; pos < dataTemp.size(); ++pos)
            {
                ...
                if (FUND == funcNo)
                {
                    if ( record.find("产品总资产") != record.end() )
                    {
                        record["资产单元净值"] = record["产品总资产"];
                    }
                    if ( record.find("单元总资产") != record.end() )
                    {
                        record["资产单元净值"] = record["单元总资产"];
                    }
                    if ( record.find("T+1交易可用") != record.end() )
                    {
                        record["T+1交易可用金额"] = record["T+1交易可用"];
                    }

                    if ( record.find("T+0交易可用") != record.end() )
                    {
                        record["T+0交易可用金额"] = record["T+0交易可用"];
                    }
                }

                string AccountID = XtAgentConfig::instance()->getAccountID(record["产品名称"]);
                if (!AccountID.empty())
                {
