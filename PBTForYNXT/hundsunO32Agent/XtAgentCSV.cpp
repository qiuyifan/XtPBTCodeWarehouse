...
namespace agent
{
    ...
    // 删除字符串中所有的""以及""内部的,号
    string delCSVFunkingUnicodeDecimal(string& src)
    {
        bool inQuotes = false;
    ...
    void XtAgentCSVApi::getDataFromFile(const string& file, const int& funcNo, map < string, vector< map< string, string > > >& retData)
    {
        ...
        if (!dataTemp.empty())
        {
            ...
            {
                //string line = dataTemp[pos].substr(3, (dataTemp[pos]).size() -6);
                //loadNamePos(line ,namepos);
            ...
            for (int pos = headLenth; pos < dataTemp.size(); ++pos)
            {
                ...
                //string line = dataTemp[pos].substr(3, (dataTemp[pos]).size() -6);
                //split(lineParm, line, "\"\"\",\"\"\"");
                string tmp = delCSVFunkingUnicodeDecimal(dataTemp[pos]);
                split(lineParm, tmp, ",");
                ...
                string AccountID ;
                map<string, string>::iterator iter = record.find("基金编号");
                if (iter != record.end())
                {
                    AccountID = record["基金编号"];
                }

                if (!AccountID.empty())
                {
                    size_t position = AccountID.find(".");
                    if (position != AccountID.npos)
                    {
                        AccountID = AccountID.substr(0, position);
                    }
